...
function M.get_word_for_translate(cursor_pos, buff_id)
  ...
  local word_after = ""
  for i = 1, 2, 1 do
    local _pos = { cursor_pos[1], word_under_cursor_pos.word_start - 2 }
    ...
    _pos = { cursor_pos[1], word_under_cursor_pos.word_end + 2 }
    vim.fn.cursor(unpack(_pos))
  ...
  for i = 1, 2, 1 do
    ...
    if CURRENT_DICTIONARY[M.get_dictionary_word(words_before .. _word)] ~= nil then
      ...
      word_under_cursor_pos = M.get_word_position(vim.fn.expand("<cword>"),
        { cursor_pos[1], word_under_cursor_pos.word_start - 2 }, buff_id)
    end
  end
  ...
  for i = 1, 2, 1 do
    ...
    if CURRENT_DICTIONARY[M.get_dictionary_word(_word .. words_after)] ~= nil then
      ...
      word_under_cursor_pos = M.get_word_position(vim.fn.expand("<cword>"),
        { cursor_pos[1], word_under_cursor_pos.word_end + 2 }, buff_id)
    end
  end
      ...
      if not CURRENT_DICTIONARY[M.get_dictionary_word(word)] then
        ...
        while CURRENT_DICTIONARY[M.get_dictionary_word(word)][i] ~= nil do
          if CURRENT_DICTIONARY[M.get_dictionary_word(word)][i].translate == translate then
            translate_colour = CURRENT_DICTIONARY[M.get_dictionary_word(word)][i]
                .colour
            i = 1
            break
          end
        ...
        if translate_colour ~= nil then
          ...
          vim.api.nvim_buf_add_highlight(buff_id, 111, "TranslateHighlight" .. COUNT,
            line_num, index[1] - 1,
            index[1] + #word + #translate + 3)
          vim.api.nvim_set_hl_ns(111)
          ...
          vim.api.nvim_set_hl(111, "TranslateHighlightDefault",
            { fg = M.NON_ACTIVE_TRANSLATE_COLOUR })
          vim.api.nvim_buf_add_highlight(buff_id, 111, "TranslateHighlightDefault",
            line_num, index[1] - 1,
            index[1] + #word + #translate + 3)
          vim.api.nvim_set_hl_ns(111)
...
function M.delete_translated_word()
  ...
  local word = line:sub(0, cursor_position[2]):match("[^%[]*$")
      .. line:sub(cursor_position[2] + 1, -1):match("[^%]]*")
